...
class TestSplitNodesDelimiter(unittest.TestCase):
    def test_bold(self):
        ...
        self.assertEqual(
            new_node,
            [
                TextNode("This should test ", TextType.TEXT),
    ...
    def test_first_and_last_delimiter(self):
        node = TextNode(
            "**ANNOUNCEMENT** For all users! We are having a **SALE!**", TextType.TEXT
        )
        new_node = split_nodes_delimiter([node], "**", TextType.BOLD)
        self.assertEqual(
            new_node,
            [
                TextNode("ANNOUNCEMENT", TextType.BOLD),
                TextNode(" For all users! We are having a ", TextType.TEXT),
                TextNode("SALE!", TextType.BOLD),
            ],
        )
    ...
    def test_multiple_nodes(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                TextNode("warning", TextType.ITALIC),
                TextNode(" for something", TextType.TEXT),
                TextNode(
                    "All warnings should be taken very seriously.", TextType.ITALIC
                ),
            ],
        )
    ...
    def test_multiple_occurances(self):
        node = TextNode(
            "This **text** contains **multiple** different string **types.**",
            TextType.TEXT,
        )
        new_node = split_nodes_delimiter([node], "**", TextType.BOLD)
        self.assertEqual(
            ...
            [
                ...
                TextNode(" different string ", TextType.TEXT),
    ...
    def test_multiple_nodes_and_occurances(self):
        nodes = [
            TextNode(
                "**LISTEN UP PRIVATE!** No shinanigans will be **TOLERATED** on this vessel!",
                TextType.TEXT,
            ),
            TextNode("Did you hear me?", TextType.BOLD),
            TextNode(
                "I SAID **NO SHENANIGANS!** ON MY **DAMN VESSEL!** YA HEAR ME?!",
                TextType.TEXT,
            ),
        ]
        ...
        self.assertEqual(
            new_nodes,
            [
                TextNode("LISTEN UP PRIVATE!", TextType.BOLD),
                TextNode(" No shinanigans will be ", TextType.TEXT),
                TextNode("TOLERATED", TextType.BOLD),
                TextNode(" on this vessel!", TextType.TEXT),
                TextNode("Did you hear me?", TextType.BOLD),
                TextNode("I SAID ", TextType.TEXT),
                TextNode("NO SHENANIGANS!", TextType.BOLD),
                TextNode(" ON MY ", TextType.TEXT),
                TextNode("DAMN VESSEL!", TextType.BOLD),
                TextNode(" YA HEAR ME?!", TextType.TEXT),
            ],
        )

    def test_close_delimiters(self):
        node = TextNode(
            "Testing **close** **delimiters** for testing purposes.", TextType.TEXT
        )
        new_node = split_nodes_delimiter([node], "**", TextType.BOLD)
        self.assertEqual(
            new_node,
            [
                ...
                TextNode("close", TextType.BOLD),
                TextNode(" ", TextType.TEXT),
                TextNode("delimiters", TextType.BOLD),
                TextNode(" for testing purposes.", TextType.TEXT),
            ],
        )
...
class TestSplitNodesImage(unittest.TestCase):
    ...
    def test_multiple_nodes_split_images(self):
        ...
        self.assertListEqual(
            [
                ...
                TextNode(
                    "also this one", TextType.IMAGE, "https://i.imgur.com/3elNhQu.png"
                ),
    ...
    def test_image_missing_parenthesis(self):
        node = TextNode(
            "This should ![break(https://i.imgur.com/zjjcJKZ.png)", TextType.TEXT
        )
        new_nodes = split_nodes_image([node])
        self.assertListEqual(
            [
                TextNode(
                    "This should ![break(https://i.imgur.com/zjjcJKZ.png)",
                    TextType.TEXT,
                )
            ],
            new_nodes,
    ...
    def test_image_with_special_characters(self):
        node = TextNode(
            "![I wonder* if this will break!](https://i.imgur.com/zjjcJKZ.png)",
            TextType.TEXT,
        )
        new_nodes = split_nodes_image([node])
        self.assertEqual(
    ...
    def test_image_with_non_text_node(self):
        node = TextNode(
            "This is a bold ![image](https://i.imgur.com/zjjcJKZ.png)", TextType.BOLD
        )
        new_nodes = split_nodes_image([node])
        self.assertEqual(
            [
...
class TestSplitNodesLink(unittest.TestCase):
    ...
    def test_adjacent_links(self):
        node = TextNode(
            "[link](https://amazon.com)[link](https://google.com)", TextType.TEXT
        )
        new_nodes = split_nodes_link([node])
        self.assertEqual(
            [
    ...
    def test_link_with_special_characters(self):
        node = TextNode(
            "[I wonder* if this will break!](https://amazon.com)", TextType.TEXT
        )
        new_nodes = split_nodes_link([node])
        self.assertEqual(
            [
                TextNode(
                    "I wonder* if this will break!", TextType.LINK, "https://amazon.com"
                )
            ],
            new_nodes,
        )
...
class TestTextToTextNodes(unittest.TestCase):
    ...
    def test_text_to_textnodes_just_text(self):
        ...
        self.assertEqual(
            [
                TextNode(
                    "Just some plain text without any fancy formatting.", TextType.TEXT
                )
            ],
            nodes,
        )
...
"""
    ...
    def test_extract_title_indented(self):
        md = "   # Indented Title"
        with self.assertRaises(Exception):
            extract_title(md)

    def test_extract_title_special_chars(self):
        md = "# Welcome to My Blog! (2024) @Home"
        title = extract_title(md)
        self.assertEqual(title, "Welcome to My Blog! (2024) @Home")
